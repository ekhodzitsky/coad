...
workcell:
  ...
  owns_paths:
    ...
    - tools/coad-validator/src/coad_validator/contract_update_integrity.py
...
surface:
  - name: coad check
    kind: cli
    visibility: public
    contract: Runs the single public COAD boundary/evidence check command and reuses one validation report across check source builders.
    proof:
      kind: unit-test
